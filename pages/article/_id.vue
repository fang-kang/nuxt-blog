  ...
  <div class="article_id clearfix">
    <div class="left-content" v-if="articleDetail.artTitle">
      ...
      <p class="article-info">
        <span>发布于：{{ articleDetail.cdate.split(" ")[0] }}</span
        ><span>{{ articleDetail.pv }} 次浏览</span
        ><span>{{ commentsList.total }} 条评论</span>
      </p>
    ...
    <div v-else class="left-content not-found">咦，你要找的东西好像不见了</div>

    <!--评论-->
    <comment
      :artDiscuss="articleDetail.artDiscuss"
      :commentsList="commentsList"
      :commentId="{ id: articleDetail.id }"
    ></comment>
  </div>
</template>
...
export default {
      ...
      meta: [
        {
          hid: "index",
          name: "description",
          content: this.$store.state.article.detail.abstract,
        },
      ],
    };
  },
  ...
  computed: {
    ...
    markdownRender() {
      ...
      return marked(markd.replace(/(:.*:)/g, replacer)).replace(
        /<a/g,
        '<a target="_blank"'
      );
    },
