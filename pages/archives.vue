<template>
  <article class="archives">
    ...
    <section class="time-list-wrap clearfix" v-if="artObj.total > 0">
      <div class="art-list">
        <div v-for="(item, index) in returnDateArr(artObj.list)" :key="index">
          <a :id="`#${item}`" class="times"
            ><el-divider content-position="left">YEAR-{{ item }}</el-divider></a
          >
          <ul class="art-list-detail">
            <li
              class="art-detail-item"
              v-for="(list, listIndex) in returnArtList(artObj.list, item)"
              :key="listIndex"
            >
              ...
              ><span class="views"
                ><svgicon class="" name="view"></svgicon>{{ list.pv }}</span
              >
            </li>
          </ul>
        </div>
    ...
    <section v-else class="no-data">咦，这里的线索不见了～</section>
  </article>
</template>
