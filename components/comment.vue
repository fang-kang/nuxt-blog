<template>
  <div
    class="comment"
    v-if="siteConfig.discussStatus === 1 && artDiscuss === 1"
  >
    <div class="comment-container">
      <div class="comment-title" id="tohere">
        共<span>{{ commentsList.total }}</span
        >条评论
      </div>
      ...
      <div class="comment_form">
        <el-form :model="artComment" ref="artComment">
          <input
            type="hidden"
            v-model="artComment.toemail"
            v-if="toReply != ''"
          />
          <input
            type="hidden"
            v-model="artComment.tonickname"
            v-if="toReply != ''"
          />
          <span v-if="toReply != ''" class="reply_to clearfix"
            >回复：{{ toReply }}
            <i
              class="el-icon-error"
            ...
            <div class="comment-edit-container">
              ...
              <ul class="emoji-container" v-show="showEmoji">
                <li
                  ...
                  v-html="commentsRender(item)"
                  :key="item"
                  @click="renderEmoji($event, item)"
                ></li>
              </ul>
            </div>
          </el-form-item>
          ...
          <el-form-item>
            <el-button
              ...
              type="primary"
              :loading="committing"
              @click="postArtComment('artComment')"
              >提交
            </el-button>
          </el-form-item>
        </el-form>
      ...
      <div v-if="commentsList.data.length > 0" class="comment-list-wrap">
        ...
        >
          ...
          <div class="comment-content" id="r-md-preview">
            <div
              class="comment-list replyContent-bg"
              v-if="item.oldContent != null"
            >
              <div class="clearfix">
                <a
                  :href="item.to_uweb"
      ...
      <div v-else class="no-comment">快来抢个沙发吧</div>
    </div>
  </div>
  <div v-else class="cant-comment">站长暂未开启评论</div>
</template>
...
export default {
  ...
  data() {
    let validateUrl = (rule, value, callback) => {
      let isUrl = /^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+$/i.test(
        value
      );
  ...
  mounted() {
    ...
    if (localStorage.getItem("commentUserInfo")) {
      this.artComment.email =
        JSON.parse(localStorage.getItem("commentUserInfo")).email || "";
      this.artComment.nickname =
        JSON.parse(localStorage.getItem("commentUserInfo")).nickname || "";
      this.artComment.webUrl =
        JSON.parse(localStorage.getItem("commentUserInfo")).webUrl || "";
    }
